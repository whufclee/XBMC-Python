...
def handle_db_movies(movie):
    ...
    db_movie = {'fanart': movie["art"].get('fanart', ""),
                ...
                'path': 'plugin://script.qlickplay/?info=pathplay&type=movie&db=local&id=%s' % str(movie['movieid']),
                'plot': movie.get('plot', ""),
                ...
                'Play': "",
                'trailer': 'plugin://script.qlickplay/?info=playtrailer&dbid=%s' % str(movie['movieid']),
...
def handle_db_tvshows(tvshow):
    db_tvshow = {'fanart': tvshow["art"].get('fanart', ""),
                 ...
                 'path': 'plugin://script.qlickplay/?info=pathplay&type=tv&db=local&id=%s' % str(tvshow['tvshowid']),
                 'trailer': 'plugin://script.qlickplay/?info=playtvtrailer&dbid=%s' % str(tvshow['tvshowid']),
                 'Play': "",
...
def compare_album_with_library(online_list):
    ...
    for online_item in online_list:
        for local_item in local_list:
            ...
            online_item["path"] = 'plugin://script.qlickplay/?info=playalbum&dbid=%i' % album['albumid']
            if album["thumbnail"]:

...
def handle_tmdb_movies(results=[], local_first=True, sortkey="year"):
    ...
    for movie in results:
        ...
        listitem = {'title': fetch(movie, 'title'),
                    ...
                    'id': tmdb_id,
                    'path': "plugin://script.qlickplay/?info=pathplay&type=movie&db=tmdb&id=%s" % tmdb_id,
                    'media_type': "movie",
                    'country': fetch(movie, 'original_language'),
                    'plot': fetch(movie, 'overview'),
                    'Trailer': "plugin://script.qlickplay/?info=playtrailer&id=%s" % tmdb_id,
...
def handle_tmdb_tvshows(results, local_first=True, sortkey="year"):
    ...
    for tv in results:
        ...
        newtv = {'title': fetch(tv, 'name'),
                 ...
                 'Trailer': 'plugin://script.qlickplay/?info=tvtrailer&id=%s' % tmdb_id, 
                 'year': get_year(fetch(tv, 'first_air_date')),
                 'media_type': "tv",
                 'character': fetch(tv, 'character'),
                 'path': "plugin://script.qlickplay/?info=pathplay&type=tv&db=tmdb&id=%s" % tmdb_id,
...
def handle_tmdb_misc(results):
    ...
    for item in results:
        ...
        listitem = {'title': clean_text(fetch(item, 'name')),
                    ...
                    'path': "plugin://script.qlickplay?info=listmovies&---id=%s" % fetch(item, 'id'),
...
def handle_tmdb_people(results):
    ...
    for person in results:
        ...
        newperson = {'adult': str(fetch(person, 'adult')),
                     ...
                     'path': "plugin://script.qlickplay/?info=actorinfo&id=" + str(person['id']),
...
def extended_movie_info(movie_id=None, dbid=None, cache_time=14):
    ...
    path = 'plugin://script.qlickplay/?info=youtubevideo&id=%s' % fetch(response, "id")
    trakt = get_movie_info_min(fetch(response, 'imdb_id'))
    print "QQQQQ" + str(trakt)
    if trakt != []:
    ...
    movie = {'title': fetch(response, 'title'),
             ...
             'path': "plugin://script.qlickplay/?info=pathplay&type=movie&db=tmdb&id=%s" % movie_id,
             'trailer': "plugin://script.qlickplay/?info=playtrailer&id=%s" % movie_id,
...
def extended_tvshow_info(tvshow_id=None, cache_time=7, dbid=None):
    ...
    tvshow = {'title': fetch(response, 'name'),
              ...
              'media_type': "tv",
              'path': 'plugin://script.qlickplay/?info=tvinfo&id=%s' % tmdb_id,
              'Popularity': fetch(response, 'popularity'),
              'Rating': fetch(response, 'vote_average'),
              'country': fetch(response, 'original_language'),
              ...
              'Votes': fetch(response, 'vote_count'),
              'Status': translate_status(fetch(response, 'status')),
              'path': "plugin://script.qlickplay/?info=pathplay&type=tv&db=tmdb&id=%s" % tvshow_id,
              'trailer': "plugin://script.qlickplay/?info=playtvtrailer&id=%s" % tvshow_id,
...
def extended_season_info(tvshow_id, season_number):
    ...
    season = {'SeasonDescription': clean_text(response["overview"]),
              ...
              'year': year,
              'season': season_number,
              'path': 'plugin://script.qlickplay/?info=pathplay&type=tv&db=tvdb&id=%s&season=%s' % (tvdb_id, season_number), 
...
def extended_episode_info(tvshow_id, season, episode, cache_time=7):
    ...
    answer = {"actors": handle_tmdb_people(response["credits"]["cast"]),
              "path": 'plugin://script.qlickplay/?info=pathplay&type=tv&db=tmdb&id=%s&season=%s&episode=%s' % (tvshow_id, season, episode),
              "crew": handle_tmdb_people(response["credits"]["crew"]),

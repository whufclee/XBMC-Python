...
class WindowManager(object):
    ...
    def open_tvshow_info(self, prev_window=None, tvshow_id=None, dbid=None, tvdb_id=None, imdb_id=None, name=None):
        ...
        if tvshow_id:
            ...
            try: tmdb_id = get_show_tmdb_id(tvdb_id)
            except: pass
            if not tmdb_id:
            ...
            try: tmdb_id = get_show_tmdb_id(imdb_id=imdb_id)
            except: return            
    ...
    def open_audio_list(self, prev_window=None, search_str="", filters=[], sort="relevance", filter_label="", media_type="artist"):
        """
        open audio list, deal with window stack and color
        """
        from dialogs import DialogAudioList
        if prev_window:
            try:  # TODO rework
                color = prev_window.data["general"]['ImageColor']
            except:
            ...
            color = COLORMAIN
        audio_class = DialogAudioList.get_audio_window(WindowXML)
        dialog = audio_class(u'%s-AudioList.xml' % ADDON_ID, ADDON_PATH,
                               search_str=search_str,
                               color=color,
                               filters=filters,
                               filter_label=filter_label,

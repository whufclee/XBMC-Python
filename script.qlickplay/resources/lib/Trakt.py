...
def get_trakt_calendar_shows(content):
    ...
    for day in results.iteritems():
        for episode in day[1]:
            if episode["show"]["ids"]["tmdb"] and episode["show"]["language"] == SETTING("LanguageID") or episode["show"]["language"] == "en":
                ...
                show = {'title': episode["episode"]["title"],
                        ...
                        'path': 'plugin://script.qlickplay/?info=pathplay&type=tv&db=tmdb&id=%s&season=%s&episode=%s' % (episode["show"]["ids"]["tmdb"], episode["episode"]["season"], episode["episode"]["number"]),
...
def handle_trakt_movies(results):
    ...
    for movie in results:
        movie = {'title': movie["movie"]["title"],
                 ...
                 'Trailer': 'plugin://script.qlickplay/?info=playtrailer&id=%s' % str(movie["movie"]["ids"]["tmdb"]),
                 'year': movie["movie"]["year"],
                 'id': movie["movie"]["ids"]["tmdb"],
                 'imdb_id': movie["movie"]["ids"]["imdb"],
                 'trakt_id': movie["movie"]["ids"]["trakt"],
                 'slug': movie["movie"]["ids"]["slug"],
                 'path': 'plugin://script.qlickplay/?info=pathplay&type=movie&db=tmdb&id=%s' % str(movie["movie"]["ids"]["tmdb"]), 
...
def handle_trakt_tvshows(results):
    ...
    for tvshow in results:
        ...
        show = {'title': tvshow['show']["title"],
                ...
                'Trailer': 'plugin://script.qlickplay/?info=playtvtrailer&id=%s' % str(tvshow['show']['ids']["tmdb"]),
                ...
                'path': 'plugin://script.qlickplay/?info=pathplay&type=tv&db=tmdb&id=%s' % str(tvshow['show']['ids']["tmdb"]),

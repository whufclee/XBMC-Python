    ...
    class DialogVideoList(DialogBaseList, window_type):
        ...
        def context_menu(self):
            ...
            if self.type == "tv":
                ...
                imdb_id = get_imdb_id_from_movie_id(item_id)
                #notify(header="TMDb-id: "+item_id, message=" IMDb-id: "+str(imdb_id).replace("tt",""), icon=self.listitem.getProperty("poster"))
            if self.type == "tv":
                if self.listitem.getProperty("dbid"):
                    listitems = [LANG(208)]
                else:
                    listitems = [LANG(13401) % LANG(36902)]
            else:
                listitems = [LANG(208)]
            if self.listitem.getProperty("dbid"):
                listitems += [LANG(32226)]
            else:
                listitems += [LANG(32225)]
            if self.type == "tv":
            ...
            if selection == 0:
                if self.type == "tv":
                    ...
                    if self.listitem.getProperty("dbid"):
                        show_playlist = xbmc.translatePath("special://profile/playlists/video/Meta/%s.xsp" % tvdb)
                    ...
                    if self.listitem.getProperty("dbid"):
                        ...
                        url = 'plugin://plugin.video.metalliq/movies/play/tmdb/%s/%s' % (item_id, SETTING("player_alt"))
                        PLAYER.qlickplay(url,
                                         listitem=None,
                                         dbid=0,
                                         window=self)
            if selection == 1:
                if self.type == "tv":
                    ...
                    if self.listitem.getProperty("dbid"):
                        ...
                        if os.path.exists(show_playlist):
                            try:
                                os.remove(show_playlist)
                                notify(message="Smart playlist removed",header=self.listitem.getProperty("TVShowTitle"),sound=False)
                            except:
                                pass
                        if os.path.exists(show_path):
                            try:
                                shutil.rmtree(show_path)
                                notify(message="Streamfiles removed",header=self.listitem.getProperty("TVShowTitle"),sound=False)
                        ...
                        xbmc.executebuiltin("RunPlugin(plugin://plugin.video.metalliq/tv/add_to_library/%s)" % tvdb)
                        xbmc.executebuiltin("RunScript(script.qlickplay,info=afteradd)")
                else:
                    imdb = get_imdb_id_from_db("movie", self.listitem.getProperty("dbid"))
                    if self.listitem.getProperty("dbid"):
                        import shutil

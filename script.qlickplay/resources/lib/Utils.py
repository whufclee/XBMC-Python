...
TOTALS = [["Movie", "TV", "Main", "Alternate"], ["Qlick", "Action", "Window", "Player"], ["Info", "Play", "Search", "Youtube"], ["Activatewindow", "Replacewindow", "Runplugin", "Runscript"], ["Select", "Default", "Library", "Context"],["Programs","Pictures","Music","Video"]]
trailersearches = ["official trailer", "trailer", "teaser", "promo", "sneak preview", "intro", "opening credits"]
trailersearchterms = "trailer/teaser/promo/preview/intro"
SOUNDS = SETTING("notification_sounds")

def set_advanced_mode():
    SET("advanced_mode","true")

def set_next_setting(type,name,value):
    if name.title() == "Window": return
    if type.title() == "Main" or type.title() == "Context":
        if name.title() != "Player": return
    if value == None: value = SETTING(name.lower()+"_"+type.lower())
    old_ids = custom_find(value.title(),TOTALS)
    if old_ids == -1: return
    new_id = old_ids[1] + 1
    if new_id >= 4: new_id = 0
    new_value = str(TOTALS[old_ids[0]][new_id]).lower()
    SET(str(name).lower()+"_"+str(type).lower(),new_value)
    notify(header=str(type).title()+"-"+str(name).title()+" setting",message="Set to: "+str(new_value),time=500)

def cycle(value):
    if value == None: return
    else: old_ids = custom_find(value.title(),TOTALS)
    if old_ids == -1: return
    if new_id >= 4: new_id = 0
    else: new_id = old_ids[1] + 1
    return TOTALS[old_ids[0]][new_id]

def custom_find(value,matrix):
    for list in matrix:
        if value in list:
...
def get_google_autocomplete_items(search_str, youtube=False):
    ...
    for item in result[1]:
        if is_hebrew(item):
            ...
            search_str = item
        li = {"label": item,
              "path": "plugin://script.qlickplay/?info=selectautocomplete&id=%s" % search_str}
        listitems.append(li)
    return listitems
    ...
    with codecs.open(path, encoding="utf8") as f:
        for i, line in enumerate(f.readlines()):
            if not line.startswith(search_str) or len(line) <= 2:
                continue
            li = {"label": line,
                  "path": "plugin://script.qlickplay/?info=selectautocomplete&id=%s" % line}
            listitems.append(li)
            if len(listitems) > 10:
                break
    return listitems
...
def widget_selectdialog(filter=None, string_prefix="widget"):
    ...
    tvshow = {"airedyesterday": "%s [I](Trakt.tv)[/I]" % LANG(32028),
              "premieredlastweek": "%s [I](Trakt.tv)[/I]" % LANG(32029),
...
# popularpeople
    ...
    if ret > -1:
        notify(keywords[ret])
        xbmc.executebuiltin("Skin.SetString(%s.path,plugin://script.qlickplay?info=%s)" % (string_prefix, keywords[ret]))
...
def get_favs():
    ...
    for fav in json_response["result"]["favourites"]:
        ...
        newitem = {'Label': fav["title"],
                   ...
                   'Type': fav["type"],
                   'Builtin': path,
                   'path': "plugin://script.qlickplay/?info=action&id=" + path}
...
def get_icon_panel(number):
    ...
    for i in range(1, 6):
        ...
        newitem = {'Label': xbmc.getInfoLabel("Skin.String(IconPanelItem%i.Label)" % (i + offset)).decode("utf-8"),
                   'path': "plugin://script.qlickplay/?info=action&id=" + infopanel_path.decode("utf-8"),
...
def get_weather_images():
    ...
    for i in range(1, 6):
        newitem = {'Label': "bla",
                   'path': "plugin://script.qlickplay/?info=action&id=SetFocus(22222)",
...
def convert_youtube_url(raw_string):
    ...
    if youtube_id:
        return 'plugin://script.qlickplay/?info=youtubevideo&id=%s' % youtube_id
    return ""

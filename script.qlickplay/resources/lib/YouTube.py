...
def handle_youtube_videos(results, extended=False):
    ...
    for item in results:
        ...
        video = {'thumb': thumb,
                 'youtube_id': video_id,
                 'Play': 'plugin://script.qlickplay/?info=youtubevideo&id=%s' % video_id,
                 'path': 'plugin://script.qlickplay/?info=youtubevideo&id=%s' % video_id,
...
def handle_youtube_playlists(results):
    ...
    for item in results:
        ...
        playlist = {'thumb': thumb,
                    ...
                    'Play': 'plugin://script.qlickplay/?info=youtubeplaylist&id=%s' % playlist_id,
                    'path': 'plugin://script.qlickplay/?info=youtubeplaylist&id=%s' % playlist_id,
                    'title': item["snippet"]["title"],
                    'description': item["snippet"]["description"],
                    'channel_title': item["snippet"]["channelTitle"],
...
def handle_youtube_channels(results):
    ...
    for item in results:
        ...
        channel = {'thumb': thumb,
                   ...
                   'Play': 'plugin://script.qlickplay/?info=youtubechannel&id=%s' % channel_id,
                   'path': 'plugin://script.qlickplay/?info=youtubechannel&id=%s' % channel_id,
                   'Description': item["snippet"]["description"],
                   'title': item["snippet"]["title"],
                   'Date': item["snippet"]["publishedAt"].replace("T", " ").replace(".000Z", "")[:-3]}

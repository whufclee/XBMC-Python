...
confirmExit = ADDON.getSetting('confirm.exit')
PATH        = skinpath
...
class Application(xbmcgui.WindowXML):
    ...
    def onTimer(self):
        ...
        if xbmcgui.Window(10000).getProperty('TVP_RELAUNCH') == 'true':
            xbmcgui.Window(10000).setProperty('TVP_RELAUNCH', 'false')
            self.doRelaunch()
    ...
    def onAction(self, action):
        ...
        if actionId in [ACTION_PARENT_DIR, ACTION_BACK] or buttonId in [ESC]:
            if not confirmExit or utils.DialogYesNo('Are you sure you wish to quit Community Portal?'):
                return self.onBack()

...
def Get_Activation(mode):
    ...
    mydisplay = MainMenu(
        ...
        backbutton=ADDON.getLocalizedString(30083),
        nextbutton=ADDON.getLocalizedString(30002),
        backbuttonfunction='self.close();Skip_Registration()',
        nextbuttonfunction='self.close();Check_Status("yes")',
        selectbutton=ADDON.getLocalizedString(30067),
        toggleup='',
        toggledown='',
        selectbuttonfunction='self.close();Check_Status("no")',
        toggleupfunction='',
        toggledownfunction='',
...
def Skip_Registration():
    ...
    mydisplay = MainMenu(
        ...
        nextbuttonfunction='Check_Status("yes")',
...
def Check_Status(auto):
    print"### Check_Status auto = "+auto
...
def Activate(site):
    ...
    if not 'Unknown' in wifimac and not 'Unknown' in ethmac:
        ...
        while counter <5 and success == 0:
#        try:
            counter += 1
            try:
                if site == "&s=1":
                    link = Open_URL('http://tlbb.me/boxer/addnew.php?x='+encryptme('e',urlparams)+site)
                else:
                    link = Open_URL('http://tlbb.me/boxer/addnew.php?x='+encryptme('e',urlparams))                    
            except:
                pass
            if '~' in link:
                dl,vid = link.split('~')
